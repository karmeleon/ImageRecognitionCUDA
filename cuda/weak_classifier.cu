...
__constant__ unsigned char image[3*SIZE];
...
__device__ int SAT(int x1, int y1, int x2, int y2, int img)
{
	...
	for(int j = y1; j <= y2; j++)
	{
		for(int i = x1; i <= x2; i++)
		{
			area += image[img*SIZE + j*X_SIZE + i];
		}
	}
	return area;
}

__global__ void compute(int *error, int *label, int x, int y, int w, int h, int theta, int f, int comp)
{
...
int main(int argc, char*argv[])
{
	...
	cout << "cudaMalloc error: " << cudaGetErrorString(error[0]) << endl;
	cout << "cudaMalloc error: " << cudaGetErrorString(error[1]) << endl;
	cout << "cudaMalloc error: " << cudaGetErrorString(error[2]) << endl;

	error[0] = cudaMemcpy(dev_e, e, sizeof(int), cudaMemcpyHostToDevice);
	cout << "cudaMemcpy error: " << cudaGetErrorString(error[0]) << endl;
	error[0] = cudaMemcpyToSymbol(image, num, size);
	cout << "cudaMemcpyToSymbol error: " << cudaGetErrorString(error[0]) << endl;
	//END CUDA stuffs
			...
			for(int h = 1; h <= 15 - y; h++)
			{
				for(int w = 1; w <= (15 - x)/2; w++)
				{
					...
					while(theta > 0)
					{
						...
						compute<<<dimGrid, dimBlock>>>(dev_e, label, x, y, w, h, theta, f, comp);
						cudaDeviceSynchronize();
						error[0] = cudaMemcpy(e, dev_e, 1*sizeof(int), cudaMemcpyDeviceToHost);
