...
_strong_classifier* adaboost(feature* features, unsigned char* labels, uint32_t num_features, uint32_t num_examples, uint8_t stages)
{
	...
	*global_min = INT_MAX;
	...
	for (int t = 0; t < stages; t++)
	{
		...
		_strong_classifier::_weak_classifier* tempBuf = (_strong_classifier::_weak_classifier*)malloc(sizeof(_strong_classifier::_weak_classifier) * stages);
		cuda_error[0] = cudaMemcpy(tempBuf, mirrored_strong_classifier->weak_classifier, stages * sizeof(_strong_classifier::_weak_classifier), cudaMemcpyDeviceToHost);
		strong_classifier->weak_classifier = tempBuf;

		// THIS IS A GUESS PLS CORRECT
		...
		//float* tempBuf2 = (float*)malloc(sizeof(float) * stages);
		//cuda_error[2] = cudaMemcpy(tempBuf2, mirrored_strong_classifier->alpha, stages * sizeof(float), cudaMemcpyDeviceToHost);
		//strong_classifier->alpha = tempBuf2;
